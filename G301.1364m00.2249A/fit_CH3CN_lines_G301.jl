...
const FILE_NAME_WITH_MODELS_GRID = joinpath(DATA_DIR, "grid_results_CH3CN_G301_fWHII.txt")

const model_ids = open(joinpath(DATA_DIR, "modelids_G301_fWHII.jls"), "r") do file
    deserialize(file)
end
...
const LINE_VERTICAL_LABELS = OrderedDict("14_8->13_8" => 257.209474, "14_7->13_7" => 257.283986, "14_6->13_6" => 257.348563, "14_4->13_4" => 257.447913, "14_3->13_3" => 257.482686, "14_2->13_2" => 257.507523, "14_1->13_1" => 257.522426, "14_0->13_0" => 257.527393)
...
const Au = [0.0009903951357899113, 0.0011038911882994575, 0.0012025337057184433, 0.0012860328388665293, 0.0013543043057170905, 0.0014077024761178247, 0.001445614680003145, 0.0014687663958562179, 0.0014761098924640826]
const gu = [58.0, 58.0, 116.0, 58.0, 58.0, 116.0, 58.0, 58.0, 58.0]
const Eu_cm = [381.8402, 307.5164, 243.0684, 188.5107, 143.8556, 109.1134, 84.2919, 69.3967, 64.4313]
...
const modelTbs = open(joinpath(DATA_DIR, "modelTbs_G301_fWHII.jls"), "r") do file
    deserialize(file)
end
const modelTaus = open(joinpath(DATA_DIR, "modelTaus_G301_fWHII.jls"), "r") do file
    deserialize(file)
end
...
function fit_spectrum_with_gaussians(obs_X0, obs_Y0, rms, threshold, fig, fig_row, fig_col, HII_is_at_LOS)
    obs_X = copy(obs_X0)
    obs_Y = copy(obs_Y0)
...
function get_needed_gaussians_and_vlsr(models)
    ...
    for (line_label, freq0) in LINE_VERTICAL_LABELS
        ...
        models[closest_models_ids[model_id_with_max_peak]].freq = freq0
        line_vlsr = (freq0 / models[closest_models_ids[model_id_with_max_peak]].p[2] - 1) * 3e5
        if MIN_ALLOWED_LINE_VEL_WRT_VLSR_KMS < line_vlsr - mean_vlsr < MAX_ALLOWED_LINE_VEL_WRT_VLSR_KMS
            needed_gaussians = vcat(needed_gaussians, models[closest_models_ids[model_id_with_max_peak]])
            push!(individual_vlsr, line_vlsr)
            push!(individual_vlsr_err, models[closest_models_ids[model_id_with_max_peak]].err_p[2] / freq0 * 3e5)
        end
...
function get_gaussians(obs_lines_freq, obs_lines, rms, cell_id)
    ...
    axes_arr = []
    HII_is_at_LOS = false
    num_of_emission_lines = 0
    num_of_absorption_lines = 0
    for line in obs_lines
        index_of_max_flux = argmax(abs.(line))
        if line[index_of_max_flux] > THRESHOLD_IN_SIGMAS * rms
            num_of_emission_lines += 1
        end
        if line[index_of_max_flux] < -THRESHOLD_IN_SIGMAS * rms
            num_of_absorption_lines += 1
        end
    end
    if num_of_absorption_lines > num_of_emission_lines
        HII_is_at_LOS = true
    end
    for (xaxis, line) in zip(obs_lines_freq, obs_lines)
        fig_row, fig_col = one_to_two_dim(plot_id)
        line_models, axs = fit_spectrum_with_gaussians(xaxis, line, rms, THRESHOLD_IN_SIGMAS * rms, fig, fig_row, fig_col, HII_is_at_LOS)
...
function get_phys_model(logN_dV0, lognH0, Tg0, fHII0, turnFreq0, line_list)
...
function get_closest_phys_model(obs, inv_sigma_obs, line_list, absent_lines_list, threshold, max_Tgas, fwhm, fill_fac, HII_is_at_LOS)
    function get_multidim_indices(model_index_given)
        ...
        S %= len_nH * len_Tg * len_fHII * len_Freq
        index_lognH = div(S, (len_Tg * len_fHII * len_Freq)) + 1
        S %= len_Tg * len_fHII * len_Freq
        index_Tg = div(S, (len_fHII * len_Freq)) + 1
        S %= len_fHII * len_Freq
...
function emcee_phys(obs::Vector{Float64}, inv_sigma_obs::Vector{Float64}, ln2pisigma::Vector{Float64}, threshold::Vector{Float64}, line_list::Vector{Int64}, absent_lines_list::Vector{Int64}, max_Tgas::Float64, fwhm::Float64, HII_is_at_LOS::Bool, fill_fac_in::Float64 = 1.0, fill_fac_err::Float64 = Inf)
    ...
    function log_prob_phys(p)
        ...
        if Tg0 < GAS_TEMPERATURES[1] || Tg0 > GAS_TEMPERATURES[end] || Tg0 > max_Tgas || Tg0 > MAX_GAS_TEMPERATURE
            return -Inf
        end
...
function get_physical_conditions(cell)
    ...
    cell.fHII = median(@view(flat_samples[4, :]))
    cell.turnFreq = median(@view(flat_samples[5, :]))
    cell.fill_fac = median(@view(flat_samples[6, :]))
    ...
    cell.err_fHII = get_phys_err(@view(flat_samples[4, :]))
...
function main()
    ...
    open(joinpath(DATA_DIR, "cells_G301.jls"), "w") do file
    ...
    image_fHII = copy(image_data[:, :, 1]) ; image_fHII .= NaN
    image_nu0 = copy(image_data[:, :, 1]) ; image_nu0 .= NaN
    ...
    image_errfHII = copy(image_data[:, :, 1]) ; image_errfHII .= NaN
    image_errnu0 = copy(image_data[:, :, 1]) ; image_errnu0 .= NaN
    ...
    write_into_fits_file(image_NdV, "image_NdV_G301.fits", header_to_write)
    header_to_write["BTYPE"] = "Nrot"
    header_to_write["BUNIT"] = "cm^-3 s"
    write_into_fits_file(image_N_rot, "image_Nrot_G301.fits", header_to_write)
    header_to_write["BTYPE"] = "log(N)"
    header_to_write["BUNIT"] = "[cm^-2]"
    write_into_fits_file(image_N, "image_N_G301.fits", header_to_write)
    header_to_write["BTYPE"] = "log(nH2)"
    header_to_write["BUNIT"] = "[cm^-3]"
    write_into_fits_file(image_nH2, "image_nH2_G301.fits", header_to_write)
    header_to_write["BTYPE"] = "Tgas"
    header_to_write["BUNIT"] = "K"
    write_into_fits_file(image_Tgas, "image_Tgas_G301.fits", header_to_write)
    header_to_write["BTYPE"] = "Trot"
    header_to_write["BUNIT"] = "K"
    write_into_fits_file(image_T_rot, "image_Trot_G301.fits", header_to_write)
    header_to_write["BTYPE"] = "tau"
    header_to_write["BUNIT"] = "tau"
    write_into_fits_file(image_tau, "image_tau_G301.fits", header_to_write)
    header_to_write["BTYPE"] = "tau"
    header_to_write["BUNIT"] = "tau"
    write_into_fits_file(image_mintau, "image_mintau_G301.fits", header_to_write)
    header_to_write["BTYPE"] = "VLSR"
    header_to_write["BUNIT"] = "km/s"
    write_into_fits_file(image_vlsr, "image_vlsr_G301.fits", header_to_write)
    header_to_write["BTYPE"] = "cell id"
    header_to_write["BUNIT"] = "cell id"
    write_into_fits_file(image_cellid, "image_cellid_G301.fits", header_to_write)
    header_to_write["BTYPE"] = "number of lines"
    header_to_write["BUNIT"] = "number of lines"
    write_into_fits_file(image_num_lines, "image_num_lines_G301.fits", header_to_write)
    header_to_write["BTYPE"] = "FWHM"
    header_to_write["BUNIT"] = "km/s"
    write_into_fits_file(image_fwhm, "image_fwhm_G301.fits", header_to_write)
    header_to_write["BTYPE"] = "cloud size"
    header_to_write["BUNIT"] = "AU"
    write_into_fits_file(image_cloud_size, "image_cloud_size_G301.fits", header_to_write)
    header_to_write["BTYPE"] = "max VLSR diff"
    header_to_write["BUNIT"] = "km/s"
    write_into_fits_file(image_max_vlsr_diff, "image_max_vlsr_diff_G301.fits", header_to_write)
    header_to_write["BTYPE"] = "Intensity error"
    header_to_write["BUNIT"] = "K"
    write_into_fits_file(image_rmsT, "image_rmsT_G301.fits", header_to_write)
    header_to_write["BTYPE"] = "fHII"
    header_to_write["BUNIT"] = "fHII"
    write_into_fits_file(image_fHII, "image_fHII_G301.fits", header_to_write)
    header_to_write["BTYPE"] = "EM"
    header_to_write["BUNIT"] = "[pc cm^-6]"
    write_into_fits_file(image_nu0, "image_EM_G301.fits", header_to_write)
    ...
    header_to_write["BUNIT"] = "[cm^-3 s]"
    write_into_fits_file(image_errNdV, "image_errNdV_G301.fits", header_to_write)
    header_to_write["BTYPE"] = "error Nrot"
    header_to_write["BUNIT"] = "cm^-3 s"
    write_into_fits_file(image_errNrot, "image_errNrot_G301.fits", header_to_write)
    header_to_write["BTYPE"] = "error log(nH2)"
    header_to_write["BUNIT"] = "[cm^-3]"
    write_into_fits_file(image_errnH2, "image_errnH2_G301.fits", header_to_write)
    header_to_write["BTYPE"] = "error Tgas"
    header_to_write["BUNIT"] = "K"
    write_into_fits_file(image_errTgas, "image_errTgas_G301.fits", header_to_write)
    header_to_write["BTYPE"] = "error Trot"
    header_to_write["BUNIT"] = "K"
    write_into_fits_file(image_errTrot, "image_errTrot_G301.fits", header_to_write)
    header_to_write["BTYPE"] = "error VLSR"
    header_to_write["BUNIT"] = "km/s"
    write_into_fits_file(image_errvlsr, "image_errvlsr_G301.fits", header_to_write)
    header_to_write["BTYPE"] = "error fHII"
    header_to_write["BUNIT"] = "fHII"
    write_into_fits_file(image_errfHII, "image_errfHII_G301.fits", header_to_write)
    header_to_write["BTYPE"] = "error EM"
    header_to_write["BUNIT"] = "[pc cm^-6]"
    write_into_fits_file(image_errnu0, "image_errEM_G301.fits", header_to_write)
